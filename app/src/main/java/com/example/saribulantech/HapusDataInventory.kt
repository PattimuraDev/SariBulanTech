            ...
            if (it.exists()) {
                if (it.hasChild(namaInventory)) {
                    database.child(namaInventory).removeValue().addOnSuccessListener {
                        val inputDeleteNamaInventory =
                            findViewById<EditText>(R.id.inputHapusNamaInventory)
                    ...
                    Toast.makeText(this, "Inventory $namaInventory tidak ada", Toast.LENGTH_SHORT)
                        .show()
                }
            } else {
