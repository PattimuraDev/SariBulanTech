...
class HapusDataTransaksi : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnHapusDataTransaksi.setOnClickListener {
            if (inputTahunHapusLaporan.text.toString()
                    .isNotEmpty() && inputBulanHapusLaporan.text.toString().isNotEmpty()
            ) {
                hapusDataTransaksi(
                    inputTahunHapusLaporan.text.toString(),
                    inputBulanHapusLaporan.text.toString()
                )
            } else {
            ...
            if (it.exists()) {
                if (it.hasChild(tahun)) {
                    ...
                    databaseTahunRef.child(bulan).removeValue().addOnSuccessListener {
                        ...
                        Toast.makeText(this, "Data transaksi berhasil dihapus", Toast.LENGTH_SHORT)
                            .show()
